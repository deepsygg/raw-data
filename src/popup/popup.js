// Popup Script for raw.data
// Same functionality as sidepanel but in popup mode
...
document.getElementById('togglePanelMode')?.addEventListener('click', async () => {
  ...
  chrome.runtime.sendMessage({ 
    ...
    if (response && response.success) {
      ...
      console.log('[raw.data] Close popup and click icon again');
...
// Settings and language are initialized in DOMContentLoaded above
...
async function saveToHistory(scanData, scanType) {
  try {
    ...
    const historyEntry = {
      ...
      title: scanData.meta?.title || 'Untitled',
      timestamp: Date.now(),
      elementCount: scanData.ui_elements?.length || 0,
    ...
    if (history.length > MAX_HISTORY) {
      ...
      // Delete their scan data
      const keysToRemove = removed.map(entry => `scan_${entry.id}`);
    ...
    await chrome.storage.local.set({ 
      [HISTORY_KEY]: history,
      [`scan_${scanId}`]: scanData
    });
    
    console.log('[raw.data] Scan saved to history:', scanId);
  } catch (error) {
    console.error('[raw.data] Failed to save to history:', error);
